...
d3 <- d3 %>%
  ...
  ggplot(aes(x="", y=Data.Value, fill=Indicator)) +
  geom_bar(stat="identity", width=1, color="white") +
  coord_polar("y", start=0) +
  theme_void() + 
  theme(legend.position="none") +
  
  geom_text(aes(y = Data.Value, label = Indicator), color = "white", size=6) +
...
knitr::kable(head(d3, 3), format = 'html')
```
...
death_college_clust <- Mclust(dplyr::select(combined, OD_deaths, Unemployment.raw.value))

death_unemploy_clust <- Mclust(dplyr::select(combined, OD_deaths, High.school.completion.raw.value))

death_hs_clust <- Mclust(dplyr::select(combined, OD_deaths, Some.college.raw.value))
```
